...
`save_result` には実在する元動画ファイルを渡す。最初の保存時にファイル名と SHA-256 を `catalog/<元動画名>/source.sha256` に記録し、次版保存時に照合する。同じ保存名の別動画なら保存を止め、名前変更を求める。保存済み CSV の閲覧時には元動画や解析依存物を要求しない。
