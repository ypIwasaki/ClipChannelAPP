...
> 2026-09-24追記：[複数人音声の実測](multi-speaker-validation.md)では20分のVAD・照合と先頭10分のASRを実行し、利用者が8区間を「他者のみ」2件・「声が重なる」6件と判定した。対象者のみの正解例と、重なりに対象者が含まれるかの判定はない。この人物専用の追加調整は行わず、今後の動作確認は別動画で行う。S3の完成確認方法は[固定例M-04](acceptance-examples.md)を優先する。
...
| S3 | 将来の別動画で対象者のみ・他者のみ・重なりを試聴確認した少数区間のVAD→照合→ASR | スコア・元時刻付き発言、不明と手修正、他者除外を確認。全文修正を許容し、誤り率・修正量の新たな数値上限を設けない | [20分検証](multi-speaker-validation.md)で人物再利用と照合・10分ASRを部分確認。8区間の利用者判定は他者のみ2件・重なり6件で、対象者のみの正解例はない。手修正UI・他者排除率・製品閾値は未確認 |
